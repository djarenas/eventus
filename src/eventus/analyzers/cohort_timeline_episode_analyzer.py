"""
cohort_timeline_episode_analyzer.py
CohortTimelineEpisodeAnalyzer — episode coverage analytics for one identity
...
from . import cohort_timeline_episode_analyzer_utils as utils

_ERROR = "[CohortTimelineEpisodeAnalyzer] Error"


class CohortTimelineEpisodeAnalyzer:
    """
    I compute episode coverage analytics for one episode identity within
    a CohortTimeline.

    enrich_with_episode_coverage() returns a new CohortTimeline enriched
    with eps_comp_{identity}_* columns. Always overwrites existing columns.
    ...
    call enrich_with_episode_coverage() first.
    ...
    - identity not in cohort_timeline.episode_identities
    """
    ...
    def __init__(self, cohort_timeline: CohortTimeline, identity: str) -> None:
        ...
        utils.require_identity_present(identity, cohort_timeline.episode_identities)
    ...
    def enrich_with_episode_coverage(self) -> CohortTimeline:
        """
        Return a new CohortTimeline enriched with eps_comp_{identity}_*
    ...
    def get_summary(self, percentiles: list[int] = [25, 50, 75]) -> EpisodeCoverageSummary:
        """
    ...
    def __repr__(self) -> str:
        return (
            f"CohortTimelineEpisodeAnalyzer(\n"
