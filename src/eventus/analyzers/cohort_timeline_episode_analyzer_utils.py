"""
cohort_timeline_episode_analyzer_utils.py
All utilities for CohortTimelineEpisodeAnalyzer — guards, column helpers,
...
_ERROR   = "[CohortTimelineEpisodeAnalyzer] Error"
_WARNING = "[CohortTimelineEpisodeAnalyzer] Warning"
...
def eps_comp_col(identity: str, stat: str) -> str: return f"eps_comp_{identity}_{stat}"

# Computed coverage column helpers
def active_col(identity: str) -> str:  return eps_comp_col(identity, "active_days")
def inactive_col(identity: str) -> str: return eps_comp_col(identity, "inactive_days")
def before_col(identity: str) -> str:  return eps_comp_col(identity, "inactive_days_before_first_episode")
def after_col(identity: str) -> str:   return eps_comp_col(identity, "inactive_days_after_last_episode")
def middle_col(identity: str) -> str:  return eps_comp_col(identity, "inactive_days_middle")
def first_col(identity: str) -> str:   return eps_comp_col(identity, "first_start")
def last_col(identity: str) -> str:    return eps_comp_col(identity, "last_end")
...
def require_obs_period(has_obs_period: bool) -> None:
    if not has_obs_period:
        raise ValueError(
            ...
            f"obs_start and obs_end are required to compute episode coverage."
        )

def require_identity_present(identity: str, episode_identities: list[str]) -> None:
    if identity not in episode_identities:
        raise ValueError(
            f"{_ERROR} identity '{identity}' not found in "
            f"episode_identities: {episode_identities}"
        )
...
def require_coverage_exists(identity: str, columns: list[str], method: str) -> None:
    if not is_coverage_computed(identity, columns):
        raise ValueError(
            ...
            f"Call enrich_with_episode_coverage() first."
        )
...
def _parse_intervals(row: pd.Series, identity: str) -> list[tuple[pd.Timestamp, pd.Timestamp]]:
    ...
    (start, end) Timestamp pairs. Returns [] if no episodes.
    """
...
def _compute_entity_coverage(
    ...
    return {
        ...
        "inactive_days_before_first_episode": float(max(before_days, 0)),
        "inactive_days_after_last_episode":   float(max(after_days,  0)),
...
def compute_coverage(
    ...
    """
    Compute active/inactive day columns for eps_{identity} within each
    entity's observation period. Always overwrites existing eps_comp_
    columns for this identity.

    Returns the input DataFrame with eps_comp_{identity}_* columns
    appended or overwritten. All columns are NA for entities with no
    episodes in their obs period.
    """
    out = data.copy()
    ...
    out[before_col(identity)]   = out[entity_col].map(lambda e: results[e]["inactive_days_before_first_episode"] if results.get(e) else pd.NA)
    out[after_col(identity)]    = out[entity_col].map(lambda e: results[e]["inactive_days_after_last_episode"]   if results.get(e) else pd.NA)
...
def _build_relative_intervals(
    ...
    covered = data[
        data[eps_starts_col(identity)].notna() &
        data[eps_ends_col(identity)].notna()
    ].copy()
...
def calc_activity_over_time(
    ...
    EpisodeActivityOverTime result object.
    """
    ...
    return EpisodeActivityOverTime(data=ts, mode=mode, cohort_start=cohort_start)
...
def calc_tier3(
    ...
    return {
        ...
        f"t3_inactive_days_before_first_episode (n={len(before_pos)})":
            _stats(before_pos[before_col(identity)], percentiles),
        f"t3_inactive_days_after_last_episode (n={len(after_pos)})":
...
def calc_summary(
    ...
    """
    Compute all three summary tiers and return an EpisodeCoverageSummary.
    Requires coverage columns to already exist on data.
    """
    return EpisodeCoverageSummary(
        identity = identity,
