"""
occurrence_stats_utils.py
Per-entity occurrence stat computation for CohortTimelineOccurrenceAnalyzer.
Imports primitives from occurrence_primitives_utils.
...
compute_timing_stats(series, obs_start, obs_end, max_n)
    Per-entity timing of the nth occurrence and recency.
...
from .occurrence_primitives_utils import parse_dates, compute_gaps
...
def compute_volume_stats(
    ...
    ----------
    series : pd.Series
        Raw occ_{identity} column (pipe-delimited date strings).
    obs_start : pd.Series
...
def compute_timing_stats(
    ...
    """
    Compute per-entity timing of the nth occurrence relative to obs_start,
    ...
    pd.DataFrame
        ...
        NaN where entity has fewer occurrences than nth.
        recency_days is NaN for entities with zero occurrences.
    """
    rows = []
    for val, s, e in zip(series, obs_start, obs_end):
        dates = parse_dates(val, s, e)
...
def _shape_row(
    ...
    # weighted average day of occurrence, normalized to [0, 1]
