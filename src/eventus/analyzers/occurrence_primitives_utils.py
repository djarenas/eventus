"""
occurrence_primitives_utils.py
Primitive operations shared across all occurrence analysis utils.
...
def parse_dates(
    ...
    """
    Parse a pipe-delimited occurrence date string into a sorted list
    of pd.Timestamps, retaining only dates within [obs_start, obs_end].

    Parameters
    ----------
    value : Any
        Raw cell value from the occ_{identity} column.
...
def compute_gaps(dates: list[pd.Timestamp]) -> np.ndarray:
    ...
    dates : list[pd.Timestamp]
        Sorted list of occurrence timestamps. Must have at least 2
