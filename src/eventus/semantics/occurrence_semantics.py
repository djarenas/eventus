...
OccurrenceSemantics — maps generic concepts to specific column names
in occurrence (point-in-time) data.
"""
...
class OccurrenceSemantics:
    """
    I am a description of what columns mean in occurrence data.
    ...
    ----------
    identity : str | None
        What kind of occurrences these are. Letters, numbers, and
    ...
    date_col : str
        Column for the occurrence date.

    also_defined_by : list[str] | None
        Columns that are part of the occurrence's identity — two
        occurrences on the same date can only be deduplicated if all
    ...
    descriptor_cols : dict[str, DescriptorColConfig] | None
        Columns that describe the occurrence but are not part of its
    ...
    occurrence_id_col : str | None
        Optional column for a unique occurrence identifier.
    ...
    @classmethod
    def build_from_yaml(cls, path) -> "OccurrenceSemantics":
        """
        Build an OccurrenceSemantics from a YAML file.
    ...
    def to_yaml(self, path) -> None:
        ...
        if self.occurrence_id_col:
            out["occurrence_id_col"] = self.occurrence_id_col
