"""
event_duration_histogram_plotter.py
EventDurationHistogramPlotter — histogram and KDE plots of event
durations from an EventDurationResult.

No stratification — use EventDurationViolinPlotter for group comparisons.
...
_ERROR = "[EventDurationHistogramPlotter] Error"


class EventDurationHistogramPlotter:
    """
    Histogram and KDE density curve plots of event durations.
    ...
    def __init__(
        ...
        if not isinstance(config, EventDurationPlotConfig):
            raise TypeError(
                f"{_ERROR}: config must be an EventDurationPlotConfig, "
