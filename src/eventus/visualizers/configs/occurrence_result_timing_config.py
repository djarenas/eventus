"""
occurrence_result_timing_config.py
...
class FacetConfig:
    """
    Subplot layout settings for OccurrenceResultTimingPlotter.plot_histogram().

    One subplot is created per nth occurrence, stacked vertically.
...
@dataclass
class OccurrenceResultTimingConfig(BasePlotConfig):
    """
    Full configuration for OccurrenceResultTimingPlotter.
    ...
    one per nth occurrence. The plotter resolves the right config at runtime:
        cfg.histogram_per_n.get(nth, cfg.histogram)
