"""
event_duration_plot_config.py
EventDurationPlotConfig — orchestrator config for EventDurationHistogramPlotter.
...
_PREFIX = "event duration plot config"


# ── Concrete config ───────────────────────────────────────────────────────────

@dataclass
class EventDurationPlotConfig(BasePlotConfig):
    """
    Orchestrator config for EventDurationHistogramPlotter.
    ...
    Example (from dict):
        config = EventDurationPlotConfig.build_from_dict({
            "canvas": {"figsize": [10, 6]},
            "histogram": {
