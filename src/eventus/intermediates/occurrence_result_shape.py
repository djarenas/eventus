"""
occurrence_result_shape.py
OccurrenceResultShape — per-entity behavioral fingerprint result object.
Produced by CohortTimelineOccurrenceAnalyzer.compute_shape().
"""
...
_ERROR = "[OccurrenceResultShape] Error"
...
class OccurrenceResultShape(OccurrenceResult):
    """
    ...
    Produced by
    -----------
    CohortTimelineOccurrenceAnalyzer.compute_shape()
    ...
    All stats are NaN where the minimum occurrence threshold is not met.
    ...
    def n_with_gaps(self) -> int:
        """Number of entities with at least 2 occurrences (gap stats defined)."""
        return int(self._data["mean_gap"].notna().sum())

    @property
    def n_with_shape(self) -> int:
        """Number of entities with at least 3 occurrences (burstiness defined)."""
        return int(self._data["burstiness"].notna().sum())

    @property
    def n_with_memory(self) -> int:
        """Number of entities with at least 4 occurrences (memory defined)."""
