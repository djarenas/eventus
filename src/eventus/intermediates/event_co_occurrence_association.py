...
class EventCoOccurrenceAssociation:
    ...
    - Log method (Woolf) CIs for ratio measures (prevalence_ratio, odds_ratio)
    ...
    identity_a  : str
    identity_b  : str
    within_days : int — window used in compute_presence()
    """
    ...
    def _compute(self) -> None:
        ...
        self.odds_ratio_ci = _log_or_ci(self.odds_ratio, a, b, c, d)
        ...
        # ── Likelihood ratios (A as test, B as condition) ─────────────
        if (not math.isnan(sens) and not math.isnan(spec) and
                spec < 1.0 and sens > 0.0):
            self.lr_positive = sens / (1 - spec)
        else:
            self.lr_positive = float("nan")

        if (not math.isnan(sens) and not math.isnan(spec) and
                spec > 0.0 and sens < 1.0):
            self.lr_negative = (1 - sens) / spec
        else:
            self.lr_negative = float("nan")

        self.lr_positive_ci = _lr_ci(
            self.lr_positive, sens, spec, nb, n_no_b, positive=True
        )
        self.lr_negative_ci = _lr_ci(
            self.lr_negative, sens, spec, nb, n_no_b, positive=False
        )
    ...
    def disclaimer(self) -> str:
        return (
            "INTERPRETATION NOTE\n"
            "All measures are derived from cross-sectional co-occurrence data.\n"
            "They describe entities that had both events within the same\n"
            "observation period — NOT prospective incidence or causal risk.\n"
            "\n"
            "  prevalence_ratio : prevalence ratio, not incidence rate ratio\n"
            "  odds_ratio       : co-occurrence odds, not causal odds\n"
            "  sensitivity      : P(A | B) — treats A as a 'test' for B\n"
            "  specificity      : P(no A | no B) — same asymmetric framing\n"
            "  lr_positive      : meaningful only if A as test for B is intended\n"
            "  lr_negative      : same\n"
            "\n"
            "CI method: " + self.ci_method + "\n"
            "Bootstrap CIs are not yet implemented (planned for a future version)."
        )
    ...
    def __repr__(self) -> str:
        ...
        lines = [
            ...
            f"  P(B|A)                : {self._fmt_prop(self.p_b_given_a, self.p_b_given_a_ci)}",
            f"  P(B|no A)             : {self._fmt_prop(self.p_b_given_no_a, self.p_b_given_no_a_ci)}",
            f"  P(A|B)                : {self._fmt_prop(self.p_a_given_b, self.p_a_given_b_ci)}",
            f"  P(A|no B)             : {self._fmt_prop(self.p_a_given_no_b, self.p_a_given_no_b_ci)}",
            f"",
            f"  ── Association Measures ───────────────────────────────",
            f"  prevalence_ratio      : {self._fmt_measure(self.prevalence_ratio, self.prevalence_ratio_ci)}",
            f"  odds_ratio            : {self._fmt_measure(self.odds_ratio, self.odds_ratio_ci)}",
            f"",
            f"  ── Likelihood Ratios (A as test, B as condition) ──────",
            f"  sensitivity P(A|B)    : {self._fmt_prop(self.sensitivity, self.p_a_given_b_ci)}",
            f"  specificity P(¬A|¬B)  : {self._fmt_prop(self.specificity, _wilson_ci(self._n_neither, self._n_without_b))}",
            f"  LR+                   : {self._fmt_measure(self.lr_positive, self.lr_positive_ci)}",
            f"  LR-                   : {self._fmt_measure(self.lr_negative, self.lr_negative_ci)}",
            f"",
