"""
event_duration_result.py
EventDurationResult — validated result object from EventDurationAnalyzer.

One row per event. Carries duration_days and optional descriptor columns.
...
_ERROR = "[EventDurationResult] Error"

_REQUIRED_COLS = {"duration_days"}


class EventDurationResult:
    """
    I am the result of EventDurationAnalyzer.calc(). I hold per-event
    ...
    - duration_days is non-negative (guaranteed by Events causality check)
    ...
    ----------
    data : pd.DataFrame
        One row per event. Must contain entity_col and duration_days.
        May contain additional descriptor columns.
    entity_col : str
        Name of the entity identifier column.
    identity : str | None
        Event identity label from EventSemantics. May be None.
    ...
    def __init__(
        ...
        if data[entity_col].isnull().any():
            ...
            raise ValueError(
                ...
                f"Every event must have an entity identifier."
            )
