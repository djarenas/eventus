...
---------------------
The null is generated by resampling, per entity, via one of three
methods selected by EventCoOccurrenceGapAnalyzer.compute_test(null_method):

- "uniform_monte_carlo" (default): draw n_a and n_b dates uniformly over the
  observation period. Assumes uniform placement; does not preserve each
  type's own clustering (burstiness).
- "rotation": keep observed dates, shift the target sequence by a random
  within-window offset (wrapping). Preserves each type's burstiness.
- "label_permutation": reassign A/B labels over the pooled observed dates.

All three hold each entity's event count and observation window fixed.
This object stores whichever null was used (see null_method) alongside
the observed gaps.

The KS test
-----------
scipy.stats.ks_2samp compares the observed gap distribution to the null
distribution. A significant result means observed gaps are distributed
differently from what the chosen null would predict — typically shorter
(signal) rather than longer.
...
_KS_NOTE = (
    "Two-sample KS test comparing observed per-entity median gaps to an "
    "empirical null generated by one of three methods (see null_method): "
    "'uniform_monte_carlo' draws A and B dates uniformly over each entity's "
    "observation period; 'rotation' shifts the observed target sequence by "
    "a random within-window offset, preserving each type's own burstiness; "
    "'label_permutation' reassigns labels over the pooled observed dates. "
    "All preserve event counts and window length. A significant result "
    "means observed gaps differ from what the chosen null would produce — "
    "typically shorter gaps indicate temporal clustering. Does not describe "
    "mechanisms or directionality."
)
