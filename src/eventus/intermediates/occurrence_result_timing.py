"""
occurrence_result_timing.py
OccurrenceResultTiming — per-entity nth-occurrence timing result object.
Produced by CohortTimelineOccurrenceAnalyzer.compute_timing().
"""
...
_ERROR = "[OccurrenceTiming] Error"


class OccurrenceResultTiming(OccurrenceResult):
    """
    I hold per-entity timing stats for one identity within a CohortTimeline.

    Produced by
    -----------
    CohortTimelineOccurrenceAnalyzer.compute_timing(max_n)

    Columns in data (beyond base)
    -----------------------------
    time_to_1, ..., time_to_{max_n} : float
        Days from obs_start to the nth occurrence.
        NaN where entity has fewer than n occurrences.
    recency_days : float
        Days from last occurrence to obs_end.
        NaN for entities with zero occurrences.
    ...
    - Each nth histogram plotted over the eligible sub-cohort (n >= nth),
      denominator available via OccurrenceVolume
    """
    ...
    def n_with_timing(self, nth: int) -> int:
        ...
        ----------
        nth : int
            Which nth occurrence to count. Must be between 1 and max_n.
