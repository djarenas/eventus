...
Functions
---------
compute_survival_table(time_to_event, obs_duration, ci_method)
    Compute a KM survival table from arrays of event times and
    observation durations (used as censoring times for non-events).
"""
...
def _time_to_first(
    ...
    """
    Return days from obs_start to first occurrence within the obs window.
    Returns np.nan if value is NaN or no dates fall within the window.

    Intentionally self-contained — does not import from
    occurrence_primitives_utils so that survival_result_utils remains
    reusable outside the occurrence analysis context.
    """
    if pd.isna(value):
        return np.nan
    dates = []
...
def compute_survival_table(
    ...
    Each entity either experienced the event (has a finite time_to_event)
    or was censored at their obs_duration (time_to_event is NaN).

    Parameters
    ----------
    time_to_event : np.ndarray
        1-D float array. Days from obs_start to first occurrence.
        NaN for entities with no occurrence (censored at obs_duration).
    obs_duration : np.ndarray
        1-D float array. Length of obs period in days per entity.
        Used as the censoring time for entities with NaN time_to_event.
        Must be same length as time_to_event.
    ci_method : str
    ...
    if len(time_to_event) != len(obs_duration):
        raise ValueError(
            f"{_ERROR} time_to_event and obs_duration must be the same "
            f"length, got {len(time_to_event)} and {len(obs_duration)}"
        )
    ...
    n_total = len(time_to_event)

    # Build per-entity records: (time, is_event)
    # Censored entities use obs_duration as their time
    times    = np.where(np.isnan(time_to_event), obs_duration, time_to_event)
    is_event = ~np.isnan(time_to_event)

    # Collect unique event timepoints only — censoring alone does not
    # produce a KM step
    event_times = np.unique(times[is_event])

    if len(event_times) == 0:
        # No events at all — survival stays at 1.0 throughout
        return pd.DataFrame({
            "day":        pd.array([], dtype=int),
            "n_at_risk":  pd.array([], dtype=int),
            "n_events":   pd.array([], dtype=int),
    ...
    for t in event_times:
        ...
        # n_events = entities with an event exactly at t
        n_events = int(np.sum(is_event & (times == t)))

        # n_censored = entities censored exactly at t
        # (censored at same time as event — censored after by convention)
        n_censored = int(np.sum(~is_event & (times == t)))
...
def compute_summary_stats(
    ...
    ----------
    time_to_event : np.ndarray
        Days to first occurrence. NaN = censored.
    obs_duration : np.ndarray
    ...
    """
    n_total    = len(time_to_event)
    n_events   = int(np.sum(~np.isnan(time_to_event)))
    n_censored = n_total - n_events

    # Median survival = smallest t where S(t) <= 0.5
    # Computed from the event times directly
    event_times   = np.sort(time_to_event[~np.isnan(time_to_event)])
    ...
    return {
        ...
        "event_rate_pct":   round(100 * n_events / n_total, 1) if n_total else 0.0,
    }
