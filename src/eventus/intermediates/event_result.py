"""
event_result.py
EventResult — base class for all single-identity event result objects.
...
from .event_result_utils import to_yaml_repr

_ERROR = "[EventResult] Error"
...
class EventResult(ABC):
    """
    I am the base class for all single-identity event result objects.
    I hold a per-entity DataFrame, validate it at construction, and provide
    shared properties and display logic.

    I am never instantiated directly — use EventVolume, EventTiming,
    or EventShape.
    ...
    def __init__(
        ...
        if data[entity_col].duplicated().any():
            ...
            raise ValueError(
                ...
                f"EventResult requires one row per entity."
            )
    ...
    def _validate_identity(self, identity) -> None:
        """
        Validate the identity value. Overridden by EventPairResult
