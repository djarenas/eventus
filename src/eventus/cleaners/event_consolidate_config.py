"""
event_consolidate_config.py
EventConsolidateConfig — configuration for consolidating
same-date event records within EventsCleaner.

If consolidate is None in EventsCleanerConfig, no consolidation
is performed beyond standard deduplication.
If consolidate is an EventConsolidateConfig, records sharing
the same entity + date + also_defined_by values are consolidated
into one event, with descriptor columns aggregated according
...
_ERROR = "[EventConsolidateConfig] Error"
...
@dataclass
class EventConsolidateConfig:
    """
    Configuration for consolidating same-date event records.

    Two events are eligible for consolidation only if:
      - They belong to the same entity, AND
      - They share the same date, AND
      - All EventSemantics.also_defined_by columns match
    ...
    def validate_against_semantics(
        ...
        """
        Validate EventConsolidateConfig against EventSemantics
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "EventConsolidateConfig":
        """
