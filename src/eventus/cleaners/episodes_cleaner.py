"""
episodes_cleaner.py
EpisodesCleaner — transparent, auditable cleaning of raw episode data.
Produces a validated Episodes object with a full quality report.
"""
from __future__ import annotations
import pandas as pd

from .episodes_cleaner_config import EpisodesCleanerConfig
from eventus.semantics.episode_semantics import EpisodeSemantics

_ERROR_PREFIX = "[EpisodesCleaner] Error"
...
class EpisodesCleaner:
    """
    'I am a transparent, auditable pipeline that cleans raw episode data and records every decision I make.'

    Applies configurable cleaning rules to a raw DataFrame and produces
    a validated Episodes object. Every rejected or modified row is recorded
    ...
    config : EpisodesCleanerConfig | None
        Cleaning rules. Defaults to EpisodesCleanerConfig() if not provided.

    Examples
    --------
    >>> cleaner = EpisodesCleaner(raw_df, sem)
    ...
    def __init__(
        ...
        data:      pd.DataFrame,
        semantics: EpisodeSemantics,
        config:    EpisodesCleanerConfig | None = None,
    ) -> None:
        ...
        if config is None:
            config = EpisodesCleanerConfig()
        if not isinstance(config, EpisodesCleanerConfig):
            raise TypeError(
                f"{_ERROR_PREFIX}: config must be an EpisodesCleanerConfig object, "
    ...
    def clean(self) -> "Episodes":
        """
        Apply cleaning rules and return a validated Episodes object.
        ...
        -------
        Episodes
            A validated Episodes object containing only clean rows.
        """
        from eventus.data_objects.episodes import Episodes
        from eventus.cleaners.merge_utils import merge_overlapping_episodes
        ...
        return Episodes(self._cleaned, self._semantics)
