# Chapter 8 — Event-Episode Co-event Analysis

## Vignette: ED Visits and Hospitalizations in 2022

You have two episode streams for the same cohort: emergency department
visits (point-in-time events) and inpatient hospitalizations
(interval episodes). The scientific question is not just whether members
...
axis. The reader sees co-event visually before the statistics
quantify it.
...
EventEpisodeResult:
  ...
  n_evt_only             : 249 (32.1%)
  n_episode_only           :  17 (2.2%)
  n_neither              :   0 (0.0%)
  ────────────────────────────────────────────────
  n_evt_total            : 2,379 (across all entities)
  n_evt_within           : 392
  ────────────────────────────────────────────────
  n_with_evt_to_episode    : 436 (56.3%)
  median_evt_to_episode    : 54.8 days
  n_with_episode_to_occ    : 395 (51.0%)
  median_episode_to_occ    : 60.0 days
```
...
```python
arrays = {
    "ED → next\nhospitalization": result.data["mean_days_evt_to_episode"].dropna().values,
    "Discharge → next\nED visit":  result.data["mean_days_episode_to_occ"].dropna().values,
}
eventus.ArraysViolinPlotter(arrays, violin_config).plot(
    "output/coevent_gaps_violin.png"
)
```
...
automatically as `n_evt_within` — a signal that warrants clinical
review.
...
This is the pre-admission pipeline: ED visits that episodeually lead
to inpatient care.
...
generated for only 20% of admissions, while many episode→occ pairs
...
`result.data["mean_days_evt_to_episode"]` and
`result.data["mean_days_episode_to_occ"]`.

---

## What this demonstrated

- **Two streams, one timeline** — episodes and events are cleaned
...
- **The stacked timeline is the opening argument** — before any
  statistics, the visual shows the co-event pattern. This is
  the Sistine Chapel of the vignette series: episodes, events,
  and observation periods on the same per-member axis.

- **`EventEpisodeAnalyzer` has no config** — there are no
...
- **NaN is honest** — entities with no hospitalizations have NaN in
  all episode-related gap statistics. Entities with only one qualifying
...
survival analysis, and combined episode-event visualizations.
