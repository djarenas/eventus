...
The coverage analysis found that 31.1% of members had no Medicaid
coverage during their 18-21 window, that 29.8% were continuously
covered for the full three years, and that 24.3% had gaps. A table
...
across the full three years? A stacked timeline answers all of these
...
Here, one member's bar spans 2001-2008 (ages 18-21), another's spans
...
**Problem 3 — Plotting 800 members produces an unreadable figure.**
The full cohort has 800 members. A meaningful visualization needs a
...
```python
demog_df = pd.read_csv("data/ch04_07_member_demographics_age18_21.csv")
raw_df   = pd.read_csv("data/ch04_05_medicaid_coverage_agewindow.csv")
...
in the coverage analysis used the full 800-member cohort. The visualization uses
...
labels:
  title: "Medicaid Coverage Ages 18-21 (n=50 sample)"
...
x_axis:
  mode:     normalized   # obs periods differ per member — declared explicitly
  unit:     months
  interval: 3
...
Setting `mode: normalized` explicitly tells the plotter that observation
periods differ per member and the x-axis should be aligned to each
member's personal window (day 0 = 18th birthday). Using `mode: calendar`
would raise immediately:

```
ValueError: [StackedTimelinePlotter] x_axis='calendar' requires all
entities to have the same observation period. Found 50 different
obs_start values. Use 'normalized' or 'auto' instead.
```

This is a declared decision, not a fallback. The config is the record
of why normalized mode was chosen.
...
ages 18–21 (normalized x-axis, 0m–36m). Each row is one member.
Blue: active Medicaid coverage. Orange: coverage gap between episodes.
Dark grey: inactive before first enrollment. Light grey: inactive after
last exit. White: never enrolled during the observation window.
All five segment types are independently configured in the YAML and
appear in the legend. Members who entered late, exited early, had gaps,
or were never enrolled are each visually and analytically distinct.*
...
coverage — the 31.1% from the coverage summary.
